...
export class AccordionDirective implements OnInit {
  ...
  closeOtherLinks(openLink: AccordionLinkDirective): void {
    ...
    if ((openLink.group !== 'sub-toggled' || openLink.group !== 'main-toggled') && this.countState === 1) {
      if (window.innerWidth < 768) {
        ...
        const body = document.querySelector('#pcoded');
        if (body) {
          body.setAttribute('vertical-nav-type', 'offcanvas');
        }
        ...
        const body = document.querySelector('#pcoded');
        if (body) {
          body.setAttribute('vertical-nav-type', 'collapsed');
        }
  ...
  ngOnInit(): any {
    this._router = this.router.events.pipe(filter((event: any) => event instanceof NavigationEnd)).subscribe((event: NavigationEnd) => {
      this.countState = 0;
      this.navlinks.forEach((link: AccordionLinkDirective) => {
        if (link.group) {
